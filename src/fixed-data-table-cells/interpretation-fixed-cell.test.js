...
describe('Test InterpretationFixedCell', () => {
  const data = [{ classification: 'benign' }, { classification: 'pathogenic' }]

  it('renders without crashing', () => {
    const div = document.createElement('div')
    const element = <InterpretationFixedCell data={data} rowIndex={1} columnKey="classification" />
    ReactDOM.render(element, div)
  })

  it('renders the first record with rowIndex = 0', () => {
    const element = <InterpretationFixedCell data={data} rowIndex={0} columnKey="classification" />
