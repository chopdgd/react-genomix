...
import { TextCell, TextColumn } from '../index'

describe('Test TextColumn', () => {
  it('cellDataGetter returns expected content', () => {
    const rowData = { name: 'mike', value: '1' }
    const dataKey = 'name'
    const element = <TextColumn label="test" dataKey={dataKey} width={100} />
  ...
  it('cellRenderer returns expected content', () => {
    const cellData = 'mike'
    const rowIndex = 1
    const element = <TextColumn label="test" dataKey="test" width={100} />
    const wrapper = shallow(element)
    expect(
      wrapper
        .find('Column')
  ...
  it('headerRenderer returns expected content', () => {
    ...
    const element = <TextColumn label={label} dataKey="test" width={100} />
    const wrapper = shallow(element)
    expect(
      wrapper
        .find('Column')
