...
describe('Test LinkColumn', () => {
  it('cellDataGetter returns expected content', () => {
    ...
    const element = <LinkColumn label="test" dataKey={dataKey} width={100} columnData={{ urlBuilder }} />
  ...
  it('cellRenderer returns expected content', () => {
    ...
    const element = <LinkColumn label="test" dataKey="test" width={100} columnData={columnData} />
    const wrapper = shallow(element)
    expect(
      wrapper
        .find('Column')
        .props()
        .cellRenderer({ columnData, cellData, rowIndex })
  ...
  it('headerRenderer returns expected content', () => {
    ...
    const urlBuilder = cellData => `http://example.com/${cellData}`
    const element = <LinkColumn label={label} dataKey="test" width={100} columnData={{ urlBuilder }} />
    const wrapper = shallow(element)
    expect(
      wrapper
        .find('Column')
